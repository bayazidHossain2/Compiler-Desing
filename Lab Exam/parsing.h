    ...
    for(;pointer<tokens.size();pointer++){
        ...
        if(tokens[pointer].second=="int"||tokens[pointer].second=="float"||tokens[pointer].second=="double"||tokens[pointer].second=="char"){
            ...
            while(tokens[pointer].second!=";"){
                if(tokens[pointer+1].first=="IDENTIFIRE"){
                    ...
                    variables[tokens[pointer-1].second].second=tokens[pointer+1].second;
                    pointer+=2;
                }
                else{
                    error_message(line_number_from_token_number[pointer],"Variable name can't be : "+tokens[pointer+1].first+" "+tokens[pointer+1].second);
                }
            }
